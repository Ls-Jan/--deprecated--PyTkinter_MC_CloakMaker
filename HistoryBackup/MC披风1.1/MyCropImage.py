...
class MyCropImage:#截图（内容耦合的庞大怪物
    def __init__(self,bgCanvas):
        ...
        self.__multiple=0#截图的宽高将单位宽高的倍数，如果是自由裁剪则该值为0
    ...
    def __DrawFg(self,cropImg=False):#画顶图。如果cropImg为真则顺便对self.croppedImg赋值
        ...
        if(cropImg):
            L=self.scaleCenter[0]+int((self.coord0[0]-self.boxHalfWidth)/self.scaleRatio)
            U=self.scaleCenter[1]+int((self.coord0[1]-self.boxHalfHeight)/self.scaleRatio)
            R=self.scaleCenter[0]+int((self.coord1[0]-self.boxHalfWidth)/self.scaleRatio)
            D=self.scaleCenter[1]+int((self.coord1[1]-self.boxHalfHeight)/self.scaleRatio)
            if(L>R):
                L,R=R,L
            if(U>D):
                U,D=D,U
