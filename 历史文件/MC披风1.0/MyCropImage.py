...
class MyCropImage:#截图（内容耦合的庞大怪物
    ...
    def __DrawBg(self):#画底图，底图只在图片移动和缩放时发生变化(底图的频繁处理会造成卡顿
        ...
        self.bgImg=bgImg.resize((round(bgImg.size[0]*self.scaleMultiple),round(bgImg.size[1]*self.scaleMultiple)),PIL.Image.ANTIALIAS)#改变图片尺寸
    ...
    def __DrawFg(self,cropImg=False):#画顶图。如果cropImg为真则顺便对self.croppedImg赋值
        ...
        if(cropImg):
            ...
            if(self.unitW !=0 and self.unitH != 0):
                ...
                self.croppedImg=self.croppedImg.resize((round(self.unitW*multiple),round(self.unitH*multiple)),PIL.Image.ANTIALIAS)#改变图片大小
            if(self.callBack):
                self.callBack()
