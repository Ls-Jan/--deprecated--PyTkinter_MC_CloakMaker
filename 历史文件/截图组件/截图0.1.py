...
class CheckedRect:#选中的方框
    ...
    def __FormatCoord(self,mouse):#对超过范围的坐标值进行修改，让其限定在主画布之内
        canvas=self.myCanvas.canvas
        if(mouse.X<0):
            mouse.X=0
        if(mouse.Y<0):
            mouse.Y=0
        if(mouse.X>canvas.winfo_width()):
            mouse.X=canvas.winfo_width()
        if(mouse.Y>canvas.winfo_height()):
            mouse.Y=canvas.winfo_height()
    def __DragWidget(self,type):#把函数闭包用在这妖魔的地方里，不合适。。(但没想到更合适的方法
        def Func():
            mouse=eval('self.{}.mouse'.format(type))
            self.__FormatCoord(mouse)
            if(type=="UL"):
