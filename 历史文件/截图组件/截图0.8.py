...
class CheckedRect:#选中的方框（内容耦合的庞大怪物，代码质量很差，
    ...
    def __DragScaleCenter(self):#【右键拖拽底图】移动图片的缩放中心
        ...
        self.__DrawBg()
        if(hasattr(self.fgCanvas.canvas,'place_info')):
            self.__DrawFg()
    ...
    def __DrawFg(self,cropImg=False):#画顶图。如果cropImg为真则顺便对self.croppedImg赋值
        ...
        if(cropImg):
            ...
            if(self.unitW !=0 and self.unitH != 0):
                multiple=int(max(self.croppedImg.size[0]/self.unitW,self.croppedImg.size[1]/self.unitH))
#                self.croppedImg=self.croppedImg.resize((round(self.unitW*multiple),round(self.unitH*multiple)),Image.ANTIALIAS)#改变图片大小
#                self.croppedImg=self.croppedImg.resize((500,500),Image.ANTIALIAS)#改变图片大小

            global cv2
            self.croppedTkImg=ImageTk.PhotoImage(self.croppedImg)
            cv2.delete('cpTkImg')
            cv2.create_image((350,350),image=self.croppedTkImg,tags='cpTkImg')
